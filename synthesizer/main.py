...
from synthesizer.data_transforms_synthesizer import synthesize_data_transforms, SynthesisTaskParameters
...
def main(instances_dir: str, synthesis_timeout: int):
    ...
    for filename in sorted(glob.glob(f"{instances_dir}*.json")):
        for solver in solvers:
            ...
            synthesis_problems.append(
                (synt_decls, solver,
                 SynthesisTaskParameters(
                     instance_name=instance_name,
                     synthesis_timeout=synthesis_timeout,
                     use_metadata=False, use_ite=False)))
    ...
    for synthesis_problem in synthesis_problems:
        ...
        if len(results) == 0:
            print(
                f'[WARNING] No {synthesis_problem[2].name} solutions for {synthesis_problem[0]}.')
        ...
        for r in results:
            if r is None:
                print(
                    f'{synthesis_problem[2].name} output None solution for {synthesis_problem[0]}.')
